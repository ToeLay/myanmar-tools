...
class TranslitrationTest(unittest.TestCase):
    
    def setUp(self):
        zawgyiConverter = ZawgyiConverter()
        self.u2z = zawgyiConverter.uni2zg
        self.z2u = zawgyiConverter.zg2uni
        self.znorm = zawgyiConverter.normalizeZg
        self.testCases = [
            [" ", " "], ["123", "123"], ["abc", "abc"], [".!@|", ".!@|"],
            ["\u1021\u102C\u100F\u102C\u1015\u102D\u102F\u1004\u1039\u1031\u1010\u103C",
            "\u1021\u102C\u100F\u102C\u1015\u102D\u102F\u1004\u103A\u1010\u103D\u1031"],
            ...
            ["\u1000\u0020\u1074\u0020\u1031\u1001",
            "\u1000\u1039\u1011\u0020\u1001\u1031"],
            ["ႁခြင္းခ်က﻿္မရွိ","ခြွင်းချက်မရှိ"],["ႁခ ⁠ ြင္းခ်က ​္မ ၍","ခြွင်းချက်မ ၍"],
            ["ႁခြင္းခ်က​္မရွိ","ခြွင်းချက်မရှိ"],["ႁခြင္းခ်က⁠္မရွိ","ခြွင်းချက်မရှိ"],
            ["ႁခြင္းခ်က‌္မရွိ","ခြွင်းချက်မရှိ"],["ႁခြင္းခ်က‍္မရွိ","ခြွင်းချက်မရှိ"],
            ["ခ ါဘယ်","ခါဘယျ"],["ခ ူဘယ်","ခူဘယျ"],["ခ ေဘယ်","ခ ဘေယျ"],
            ["ခ⁠ဲဘယ်","ခဲဘယျ"],["ခ⁠ဲဘယ်","ခဲဘယျ"],["ခ‍ှဘယ်","ခှဘယျ"],
            [ #66
            ...
            ["ျမန္္မာကို","မြန်မာကို"],["တာ ့မိမိရဲ့","တာ့ မိမိရဲ့"],
            ...
            "ရိုးရွင္းေသာအဂၤလိပ္ဝါက်မ်ားကိုယ္တိုင္တည္ေဆာက္ျခင္း",
            "ရိုးရှင်းသောအင်္ဂလိပ်ဝါကျများကိုယ်တိုင်တည်ဆောက်ခြင်း"],
            ...
            #"ပေကျင်းမြို့ တရုတ်ပြည်သူ့သမ္မတနိုင်ငံ",
            #"ပကေငျြးမွို့ တရုတျပွညျသူ့သမ်မတနိုငျငံ"
            #},
            #
    ...
    # Simple Z -> U
    def test_Z2UT17(self):
        z1 = "\u1011\u1039"
        u1 = "\u1011\u103a"
        r1 = self.z2u(z1)
        self.assertEqual(r1, u1)

    def test_U2ZT1(self):
        z1 = "\u1011\u1039"
        u1 = "\u1011\u103a"
        r1 = self.u2z(u1)
        self.assertEqual(r1, z1)

    def test_ZNorm(self):
        z1 = "အကာင္ ့"
        z1NormExpected = "\u1021\u1000\u102c\u1004\u1039\u1037"  # "အကာင္";
        result = self.znorm(z1)
        self.assertEqual(result, z1NormExpected)
    ...
    # all Zawgyi -> Unicode test
    def test_Z2U(self):
        for testCase in self.testCases:
            zIn, expected = testCase
            actual = self.z2u(zIn)
            self.assertEqual(actual, expected)

    # all Unicode -> Zawgyi test
    def test_U2Z(self):
        # inconsitency between u2z conversion rule and zawgyi string
        for testCase in self.testCases:
            expected, uIn = testCase
            actual = self.u2z(uIn, normalize=True)
            self.assertEqual(actual, self.znorm(expected))

    # Compatibility test for Z->U conversion using golden file.
    def test_Z2UCompatibility(self):
        sourceResourcePath = "mmgov_zawgyi_src.txt"
        goldenResourcePath = "mmgov_unicode_out.txt"

        inputLines = resource_string('myanmar_tools.resources', sourceResourcePath).decode('utf-8').split('\n')
        goldenLines = resource_string('myanmar_tools.resources', goldenResourcePath).decode('utf-8').split('\n')

        for zLine, goldenLine in zip(inputLines, goldenLines):
            if zLine != "" and goldenLine != "":
                convertedLine = self.z2u(zLine)
                self.assertEqual(convertedLine, goldenLine)

    # Compatibility test for U->Z conversion using golden file.
    # Note that normalization of Zawgyi is needed for comparison.
    def test_U2ZCompatability(self):
        sourceResourcePath = "udhr_mya_unicode_src.txt"
        goldenResourcePath = "udhr_mya_zawgyi_out.txt"

        inputLines = resource_string('myanmar_tools.resources', sourceResourcePath).decode('utf-8').split('\n')
        goldenLines = resource_string('myanmar_tools.resources', goldenResourcePath).decode('utf-8').split('\n')

        for uLine, goldenLine in zip(inputLines, goldenLines):
            if uLine != "" and goldenLine != "":
                convertedLine = self.u2z(uLine, normalize=True)
                goldenLine = self.znorm(goldenLine)
                self.assertEqual(convertedLine, goldenLine)
